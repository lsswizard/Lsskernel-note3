...
rm -rf build
mkdir build
ARCH=arm make -s -i -C $RDIR O=build lss_defconfig menuconfig
echo "Showing differences between old config and new config"
echo "-----------------------------------------------------"
command -v colordiff >/dev/null 2>&1 && {
	diff -Bwu --label "old config" arch/arm/configs/lss_defconfig --label "new config" build/.config | colordiff
} || {
	diff -Bwu --label "old config" arch/arm/configs/lss_defconfig --label "new config" build/.config
	echo "-----------------------------------------------------"
...
case $option in
y|Y)
	cp build/.config arch/arm/configs/lss_defconfig
	echo "Copied new config to arch/arm/configs/lss_defconfig"
	;;
*)
